...
3. If `bag-info.txt` contains the element `Is-Version-Of` there MUST be a dataset in the target Data Station with the  
   ...
   (a) it has a `dansSwordToken` with the same value as `Is-Version-Of` (b) it has a `dansOtherId` with the same value
   as bag-info.txt's `Has-Organizational-Identifier` (or both are absent) (c)  the user account in  
