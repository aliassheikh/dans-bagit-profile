...
7. (a) The `bag-info.txt` file MAY contain at most one element called `Has-Organizational-Identifier`. This value may be
   used to identify duplicates and sequences. (b) If this `Has-Organizational-Identifier` is given, an additional
...
1. If the file `metadata/files.xml` declares the [DANS bag file metadata schema] namespace then it MUST adhere to that
   schema.
