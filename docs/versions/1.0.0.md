...
* **bag-sequence**: a sequence of *DANS-bag*s, that together represent the history of an archived *dataset* in the *DANS
  Data Vault*.
...
* **depositor**: the agent sending data to DANS for archiving in the *DANS Data Vault*. This term corresponds to the
  Producer in the [OAIS Reference Model].
...
* **revision**: a *DANS-bag* that has been successfully archived in the *DANS Data Vault*. It has a [UUID] identifier
  and a timestamp that records its place in the *bag-sequence*.
...
7. (a) The `bag-info.txt` file MAY contain at most one element called `Has-Organizational-Identifier`. This value may be
   used to identify duplicates <!-- TODO: 'in' ? --> and sequences. (b) If this `Has-Organizational-Identifier` is
   given, an additional
...
2. The `metadata` directory MUST contain the files: (a) `metadata/dataset.xml` and (b) `metadata/files.xml`. (c)
   **(AIP)** It MAY contain the files `metadata/amd.xml`, `metadata/emd.xml` or `metadata/license.txt`.
...
2. It MUST be formatted as `<physical-bag-relative-path><whitespace><orignal-bag-relative-path>`.
    ...
    * `<original-bag-relative-path>` MUST correspond one-on-one to a `filepath` attribute in the `file` elements in
      the `files.xml`.
...
2. The file `metadata/dataset.xml` MUST have exactly one `dcterms:license` element as a child of the
   ...
   of the [URIs of approved licenses].

3. **(AIP)** The file `metadata/dataset.xml` MUST contain at least one `dcterms:identifier` element with an
   attribute `xsi:type="id-type:URN"`. The text of this element MUST be a syntactically valid [urn:NBN] identifier which
   SHOULD be resolvable.

4. Any `dcx-dai:DAI` elements MUST contain a syntactically valid [Digital Author Identifier] (DAI) with a valid check
   digit. This DAI SHOULD refer to an existing author. The DAI MAY be provided as a `info:eu-repo/dai/nl/` URI.
...
7. Any `gml:Point`, `gml:lowerCorner` and `gml:upperCorner` elements MUST have at least two values. These values MUST be
   numeric. In case of RD (http://www.opengis.net/def/crs/EPSG/0/28992) the values should be within the valid range.
...
1. The file `metadata/files.xml` [DANS bag file metadata schema] MUST adhere to [DANS bag file metadata schema].

<!-- TODO: delete the rules below that are already covered by the schema -->
...
7. Each `file` element MAY contain any number of other [DC] or [DCTERMS] elements describing the file. It MUST NOT
   contain elements from other namespaces, unless permitted by the [DANS bag file metadata schema] schema.
   <!-- Skip if XSD is used -->
...
2. If `bag-info.txt` contains the element `Is-Version-Of` the UUID in its value MUST be the bag-id of an existing,
...
5. The accessibility and visibility of a dataset file MUST NOT be set to more restrictive than in previous, active
   ...
   Explanation: the previous revisions are the other bags in the same bag-sequence with a value in their `bag-info.txt`'
   s  `Created`-element, that is earlier than the `Created`-value in the bag under validation. For each file specified
   in `metadata/files.xml`, the corresponding bag-files in the previous revisions MUST NOT have an effective
   accessibility or effective visibility that are more open than the accessibility of that file. The accessibility and
   visibility for a file are found in the `file` element with a
   `filepath` attribute matching the bag local path of the file, in the `files:accessibleToRights`
   and `files:visibleToRights` element texts, respectively. The possible values, with increasing restrictiveness
   are: `ANONYMOUS`, `RESTRICTED_REQUEST`, `NONE`.

   If the `files:accessibleToRights` and/or `files:visibleToRights` elements are not specified, the effective value can
   be derived from the
