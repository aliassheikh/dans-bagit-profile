...
1. **(AIP)** The file `metadata/amd.xml` is optional. If it is given it MUST adhere to [DANS administrative metadata].
 
#### 3.7 `metadata/emd.xml`

1. **(AIP)** The file `metadata/emd.xml` is optional. If it is given it MUST adhere to [DANS EASY metadata dataset].
...
* [DANS deposit agreement metadata] - schema for deposit agreement metadata.
...
[DANS dataset metadata schema]: https://easy.dans.knaw.nl/schemas/md/ddm.xsd
